...
function PartnerMbtiPage() {
  ...
  const parseExistingMbti = (): MbtiSelections => {
    if (data.otherPersonalityType && data.otherPersonalityType.length === 4) {
      return {
        energy: data.otherPersonalityType[0] as EnergyType,
        perception: data.otherPersonalityType[1] as PerceptionType,
        judgment: data.otherPersonalityType[2] as JudgmentType,
        lifestyle: data.otherPersonalityType[3] as LifestyleType,
      }
    }
  ...
  const handleNext = () => {
    if (isComplete) {
      updateOtherPersonalityType(getMbtiString())
      goToNextStep()
    }
  }
